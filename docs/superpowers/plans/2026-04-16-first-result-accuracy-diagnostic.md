# First-Result Accuracy Diagnostic — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a diagnostic script that classifies every run-16 query by failure mode and produces a markdown report with failure taxonomy, score analysis, tag breakdown, and ranked hypotheses.

**Architecture:** Single standalone script (`src/scripts/diagnose-first-result.ts`) loads run 16 from `eval_runs` via `loadEvalRun`, classifies queries in-memory, computes score distributions, and writes a markdown report to `docs/analysis/`. No changes to the search pipeline or eval system.

**Tech Stack:** TypeScript, Supabase client (direct), fs (for report output). Follows existing script pattern from `batch-grade.ts`.
...
|--------|---------------------------------------------|---------------------------------------------|
| Create | `src/scripts/diagnose-first-result.ts`      | Main script: load, classify, analyze, write |
| Create | `docs/analysis/first-result-accuracy-run16.md` | Output report (generated, not hand-written) |

No existing files are modified. The script is self-contained.

---

## Task 1: Script Scaffold and Data Loading

**Files:**
- Create: `src/scripts/diagnose-first-result.ts`

- [ ] **Step 1: Create the script with Supabase setup and data loading**
...
async function main(): Promise<void> {
  ...
  // Classification, analysis, and report generation go here (Tasks 2-5)
...
Note: `classifyAll`, `generateReport`, and `writeReport` are defined in subsequent tasks. This will not compile until all tasks are done. That is intentional: we build the pieces, then verify the whole thing runs at the end.

- [ ] **Step 2: Verify the script file exists and has no syntax errors**

Run: `cd /home/adrian/repos/ledger && npx tsx --eval "import('./src/scripts/diagnose-first-result.ts')" 2>&1 | head -5`

Expected: A runtime error about `classifyAll is not defined` or similar (functions not yet written). NOT a syntax error. If you see a syntax error, fix it before proceeding.
...
**Files:**
- Modify: `src/scripts/diagnose-first-result.ts`
...
function classify(result: IPerQueryResult): IClassifiedQuery {
  ...
  // Priority 4: near miss (first correct at position 2-3)
  else if (result.position !== null && result.position >= 1 && result.position <= 2) {
    // position is 0-indexed internally but stored as the array index from scoreTestCase
    // In scoreTestCase: firstHitPosition is 0-indexed, but position stored in per_query_results
    // follows the same convention. position=1 means index 1 (second result).
    category = 'near-miss';
  }
  // Priority 5: buried (first correct at position 4-10, i.e., index 3-9)
...
- [ ] **Step 2: Verify classification logic handles the position indexing correctly**

The `position` field in `per_query_results` comes from `scoreTestCase` in `eval.ts:143-149`. It is 0-indexed (position 0 = first result). Check this:

```
position 0 → firstResultHit is true → classified as top-1-correct (handled by priority 2)
position 1 → second result → near-miss
position 2 → third result → near-miss
position 3-9 → positions 4-10 → buried
null → no hit → absent
```

The near-miss check uses `position >= 1 && position <= 2` (0-indexed positions 1-2, which are display positions 2-3). This is correct.

---

## Task 3: Score Distribution Analysis

**Files:**
- Modify: `src/scripts/diagnose-first-result.ts`

- [ ] **Step 1: Add score analysis functions**

Add below the classification functions:
...
## Task 4: Tag Breakdown Analysis

**Files:**
- Modify: `src/scripts/diagnose-first-result.ts`

- [ ] **Step 1: Add tag heatmap computation**

Add below the score analysis functions:
...
## Task 5: Report Generation and File Output

**Files:**
- Modify: `src/scripts/diagnose-first-result.ts`
- Create: `docs/analysis/` (directory, created by script)

- [ ] **Step 1: Add the report generation function**

Add below the tag analysis functions:
...
- [ ] **Step 2: Add the writeReport function and wire up main**

Add at the bottom, before the `main()` call:
...
The `main` function already calls `classifyAll`, `generateReport`, and `writeReport` from Task 1. No changes needed there.

- [ ] **Step 3: Verify the script compiles**

Run: `cd /home/adrian/repos/ledger && npx tsx --eval "import('./src/scripts/diagnose-first-result.ts')" 2>&1 | head -10`

Expected: The script should attempt to connect to Supabase and either succeed (loading run 16) or fail with a connection/auth error. No TypeScript compilation errors.

---

## Task 6: Run the Script and Verify Output

**Files:**
- Verify: `docs/analysis/first-result-accuracy-run16.md`

- [ ] **Step 1: Run the diagnostic script**

Run: `cd /home/adrian/repos/ledger && npx tsx src/scripts/diagnose-first-result.ts`

Expected output on stderr:
```
Loading eval run 16...
Loaded 144 queries from run 16.
Report written to /home/adrian/repos/ledger/docs/analysis/first-result-accuracy-run16.md
```

- [ ] **Step 2: Verify the report was generated**

Run: `head -40 /home/adrian/repos/ledger/docs/analysis/first-result-accuracy-run16.md`

Expected: A markdown file starting with `# First-Result Accuracy Diagnostic: Run 16`, containing the executive summary, failure taxonomy table, and beginning of tag breakdown. All counts should add up to 144 (or 139 scored + 5 out-of-scope).

- [ ] **Step 3: Verify category counts are consistent**

Check that:
- Top-1 correct count matches `run.first_result_accuracy * normalCases / 100` (approximately)
- All category counts for non-scope queries sum to the total non-scope count
- Out-of-scope count matches 5 (known from dashboard)

- [ ] **Step 4: Commit**

```bash
git add src/scripts/diagnose-first-result.ts docs/analysis/first-result-accuracy-run16.md
git commit -m "add first-result accuracy diagnostic script and run-16 report"
```

---

## Task 7: Review Report with Adrian

This is not a code task. After the script runs and the report is generated:

- [ ] **Step 1: Present the executive summary and failure taxonomy to Adrian**
- [ ] **Step 2: Highlight the unjudged audit (if any queries need grading)**
- [ ] **Step 3: Walk through the worst offenders**
- [ ] **Step 4: Discuss hypotheses together based on the patterns**

This is where the investigation transitions from "what's happening" to "what should we do about it."
