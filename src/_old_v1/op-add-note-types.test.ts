...
describe('opAddNote — type registration', () => {
  it('registers type and saves note when register_type is true', async () => {
    ...
    const result = await opAddNote(
      ...
      'wine-log',
      'claude-code',
      { upsert_key: 'wine-2024-malbec', description: 'Tasting notes', domain: 'general' },
      false,
      true, // register_type
    );
  ...
  it('defaults to general tier for unknown types (inferred from domain)', async () => {
    ...
    await opAddNote(
      ...
      'recipe',
      'claude-code',
      { upsert_key: 'recipe-pasta', description: 'Pasta recipe', status: 'active' },
      false,
      true,
    );
  ...
  it('registered type persists for subsequent calls', async () => {
    ...
    await opAddNote(
      ...
      'wine-log',
      'claude-code',
      { upsert_key: 'wine-1', description: 'First', domain: 'project' },
      false,
      true,
    );
