...
describe('backfillMetadata', () => {
  ...
  it('migrates knowledge-guide to general/knowledge', () => {
    const result = backfillMetadata({ type: 'knowledge-guide', delivery: 'knowledge' });
    expect(result.type).toBe('knowledge');
    expect(result.domain).toBe('general');
  });

  it('migrates knowledge-guide to project/knowledge when project-scoped', () => {
    const result = backfillMetadata({ type: 'knowledge-guide', delivery: 'knowledge', project: 'ledger' });
    expect(result.type).toBe('knowledge');
    expect(result.domain).toBe('project');
  });
  ...
  it('maps reference without project to general/reference', () => {
    const result = backfillMetadata({ type: 'reference', delivery: 'knowledge' });
    expect(result.type).toBe('reference');
    expect(result.domain).toBe('general');
  });

  it('maps reference with project to project/reference', () => {
    const result = backfillMetadata({ type: 'reference', delivery: 'knowledge', project: 'ledger' });
    expect(result.type).toBe('reference');
    expect(result.domain).toBe('project');
  });

  it('maps general to general/general', () => {
