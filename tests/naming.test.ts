...
import { validateNaming, deriveLocalFile, checkMetadataCompleteness } from '../src/lib/notes.js';
...
describe('deriveLocalFile', () => {
  it('converts hyphens to underscores and adds .md', () => {
    expect(deriveLocalFile('feedback-communication-style')).toBe('feedback_communication_style.md');
  });

  it('handles single-segment key', () => {
    expect(deriveLocalFile('profile')).toBe('profile.md');
  });

  it('handles project-scoped key', () => {
    expect(deriveLocalFile('ledger-spec-init')).toBe('ledger_spec_init.md');
  });
});
